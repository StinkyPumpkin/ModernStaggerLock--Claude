...
namespace ModernStaggerLock
{
	...
	public:
		...
		private:
			...
			{
				// Claude fork: swallow sub-threshold / in-scene StaggerStarts so
				// the actor never enters the stagger state (no animation, no noise).
				if (ShouldSuppressStagger(a_graphMgr, a_eventName)) {
					return false;
				}
	...
	private:
		...
		static bool ShouldSuppressStagger(RE::IAnimationGraphManagerHolder* a_graphMgr, const RE::BSFixedString& a_eventName);

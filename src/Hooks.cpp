...
namespace ModernStaggerLock
{
	...
	{
		auto actorRef = a_graphMgr ? skyrim_cast<RE::Actor*>(a_graphMgr) : nullptr;
		if (!actorRef || !actorRef->AsActorState()->actorState2.staggered)
			return false;
	...
	// Claude fork: decide whether to swallow a StaggerStart before MSL turns it
	// into a locked "modern" stagger. Two independent gates:
	//   1) actor is in a SexLab scene  -> never stagger (positioning shoves bodies)
	//   2) staggerMagnitude below floor -> ignore casual micro-bumps
	// Weapon-art unbalance produces a high magnitude and clears the floor, so
	// Elden Rim's intended big staggers are untouched.
	bool NotifyAnimationGraphHook::ShouldSuppressStagger(RE::IAnimationGraphManagerHolder* a_graphMgr, const RE::BSFixedString& a_eventName)
	{
		if (_strcmpi("StaggerStart", a_eventName.c_str()) != 0)
			return false;
		...
		// (1) SexLab scene guard.
		if (*settings->SuppressStaggerDuringSexScene &&
			MSLSettings::sexlabAnimatingFaction &&
			actorRef->IsInFaction(MSLSettings::sexlabAnimatingFaction)) {
			return true;
		}

		// (2) Minimum-magnitude floor.
		float staggerMagnitude = 0.f;
		actorRef->GetGraphVariableFloat("staggerMagnitude", staggerMagnitude);
		if (staggerMagnitude < static_cast<float>(*settings->MinStaggerMagnitude)) {
			return true;
		}
	...
	bool DisableStaggerJumpHook::Hook_CanProcess(RE::JumpHandler* a_handler, RE::InputEvent* a_event)
	{
		...
		if (result) {
			...
			auto settings = MSLSettings::GetSingleton();
			if (playerRef && playerRef->AsActorState()->actorState2.staggered && *settings->DisableJumpWhenStagger) {
				return false;
			}
		}
	...
	EventResult AnimEventHook::ProcessEvent(RE::BSTEventSink<RE::BSAnimationGraphEvent>* a_sink, RE::BSAnimationGraphEvent* a_event, RE::BSTEventSource<RE::BSAnimationGraphEvent>* a_eventSource)
	{
		...
		// v3.7.0: holder->As<>() is const-qualified; SetStaggerVariable mutates
		// graph vars, so cast away const (the object itself is mutable here).
		auto actorRef = const_cast<RE::Actor*>(a_event->holder->As<RE::Actor>());
		if (!actorRef) {
			return _ProcessEvent(a_sink, a_event, a_eventSource);
		}
